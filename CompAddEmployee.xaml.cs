...
namespace Employees
{
    public partial class CompAddEmployee : Page, INotifyPropertyChanged, IDataErrorInfo
    {
        ...
        private void Save_Executed(object sender, ExecutedRoutedEventArgs e)
        {
            ...
            {
                AddSpareProp(t, "SpareAddProp1", SpareProp1Value, SpareProp1Combo, args);
            }
            if (SpareProp2Name.IsVisible)
            {
                AddSpareProp(t, "SpareAddProp2", SpareProp2Value, SpareProp2Combo, args);
            }
            ...
            catch (Exception ex)
            {
                ...
                for (int i = 0; i < args1.Length; i++)
                    types += args1[i].GetType().ToString() + "\n";
